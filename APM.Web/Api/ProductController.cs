...
using APM.Web.Infrastructure.Extensions;
using APM.Web.Models;

namespace APM.Web.Api
{
    ...
    public class ProductController : ApiControllerBase
    {
        ...
        public async Task<HttpResponseMessage> Import()
        {
            ...
            int.TryParse(result.FormData["categoryId"], out int categoryId);
        ...
        private List<Product> ReadProductFromExcel(string fullPath, int categoryId)
        {
            using (var package = new ExcelPackage(new FileInfo(fullPath)))
            {
                ...
                for (int i = workSheet.Dimension.Start.Row + 1; i <= workSheet.Dimension.End.Row; i++)
                {
                    ...
                    if (int.TryParse(workSheet.Cells[i, 3].Value.ToString(), out int warranty))
                    {
                    ...
                    if (decimal.TryParse(workSheet.Cells[i, 6].Value.ToString(), out decimal promotionPrice))
                    {
